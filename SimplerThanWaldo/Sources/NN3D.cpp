...
static pair<vector<jointnames::jointnames>, vector<Point2d>> Pose_2D_to_labeled_joints(Pose_2D p) {
    ...
    vector<Point2d> joints = {
        Point2d(p.Hip_C), Point2d(p.Chest_C), Point2d(p.Head_Top),//spine
        Point2d(p.Chest_L), Point2d(p.Elbow_L), Point2d(p.Wrist_L),//l arm
        Point2d(p.Hip_L), Point2d(p.Knee_L), Point2d(p.Ankle_L),//l leg
        Point2d(p.Chest_R), Point2d(p.Elbow_R), Point2d(p.Wrist_R),//r arm
        Point2d(p.Hip_R), Point2d(p.Knee_R), Point2d(p.Ankle_R)//r leg
    };
